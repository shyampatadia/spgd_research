...
\begin{frame}{What does ``stuck at a saddle'' actually look like?}
  \begin{columns}[T,onlytextwidth]
    \column{0.50\textwidth}
      \footnotesize
      \textbf{Picture a literal horse's saddle} with a ball placed on
      it.\\[0.4em]
      Roll the ball \emph{front-to-back} --- along the dip where the
      rider would sit --- and it swings back and forth, eventually
      \alert{settling in the middle}. That middle point is the
      \alert{saddle point}: the loss looks minimised in this
      direction.\\[0.4em]
      But the ball is in \emph{unstable equilibrium.} Nudge it
      \emph{perpendicular} to that dip --- toward either flank of the
      saddle --- and it \alert{falls off drastically.} The surface
      curves sharply \emph{down} in that direction.\\[0.4em]
      So at a saddle point: gradient is zero (looks like ``done''),
      but a real descent direction exists --- and a first-order method
      can't see it. The bottom panel below shows the consequence in
      time: $\|\nabla f\|$ drops below the stagnation threshold
      $\varepsilon$ and stays there.
    \column{0.47\textwidth}
      \centering
      \includegraphics[width=\linewidth]{figures/intuition_saddle_plateau.png}\\[0.2em]
      \scriptsize{1-D cross-section through the saddle's stable
                  direction (top); resulting gradient-norm trace (bottom).}
  \end{columns}
\end{frame}

% ----- INTUITION 2: adaptive vs perturbation --- the core idea ------------
\begin{frame}{Two ways to get unstuck --- intuitively}
  \begin{block}{Imagine a recommender system that's stuck}
    Spotify keeps suggesting the same kind of song to a user who keeps
    skipping. The system's best guess is no longer improving --- it's
    \emph{stuck}. Two strategies to break out:
  \end{block}
  \vspace{0.3em}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \begin{block}{Adaptive: \emph{tune what you already know}}
        \footnotesize
        Look at history feature-by-feature: the user always finishes
        acoustic songs (small, careful steps in that direction), always
        skips electronic ones (big steps to avoid that region). Scale
        each direction by how confident you are.
        \\[0.4em]
        \alert{Adam, RMSProp, AdaGrad} do exactly this for gradient
        descent: rescale $\nabla f$ per-coordinate by past gradient
        magnitudes.\\[0.3em]
        \warn{Still only uses information you've already collected.}
      \end{block}
    \column{0.48\textwidth}
      \begin{block}{Perturbation: \emph{play a wildcard}}
        \footnotesize
        Just \emph{recommend a random song} from the whole catalogue ---
        even one that doesn't fit the user's pattern. Most get skipped.
        But occasionally, by accident, one is a banger they never would
        have found from their history alone.
        \\[0.4em]
        ...
        \\[0.3em]
        \warn{Commits to the random pick regardless of where it lands.}
      \end{block}
  \end{columns}
\end{frame}
...
\begin{frame}{The five optimisers we compare}
  ...
  \begin{block}{What each contrast isolates}
    \begin{itemize}\setlength\itemsep{0.2em}
      \item SPGD vs.\ \alert{RPGD}: \,is the \emph{selection rule} doing useful work?
...
% --- EXP 1a: setup + figure (with "why higher dims are empty" caveat) ------
\begin{frame}{Experiment 1 --- Synthetic benchmarks (setup)}
  \begin{columns}[T,onlytextwidth]
    \column{0.40\textwidth}
      \footnotesize
      \textbf{Three benchmark functions.}
      \begin{itemize}\setlength\itemsep{0.2em}
        \item \textbf{Rastrigin:} egg-carton --- hundreds of local
              minima around one global. Pure \emph{escape} problem.
        \item \textbf{Ackley:} funnel-shaped global basin + many shallow
              pits. Mixed escape/speed.
        \item \textbf{Rosenbrock:} curved banana valley, \emph{no} local
              minima. Pure \emph{speed} test.
      \end{itemize}
      \vspace{0.4em}
      \textbf{Protocol.} $d \in \{2,10,50\}$; 30 seeds per cell;
      success $\equiv f(\theta) < 10^{-2}$ from global optimum.
      \vspace{0.4em}

      \alert{Why $d{=}10$ and $d{=}50$ bars are empty:} the
      $10^{-2}$-tight target ball is simply unreachable in budget --- not
      failure, just a binary metric saturating. \emph{Loss-value}
      differences at higher $d$ \emph{do} discriminate (next slide).
    \column{0.57\textwidth}
      \centering
      \includegraphics[width=\linewidth]{figures/exp1_success_rate.png}\\[0.3em]
      \scriptsize{Success rate per (function, dimension, optimiser).
                  $d{=}10$ and $d{=}50$ are zero across the board ---
                  see caption-text on left.}
  \end{columns}
\end{frame}

% --- EXP 1b: per-benchmark interpretation ----------------------------------
\begin{frame}{Experiment 1 --- Per-benchmark interpretation}
  \footnotesize
  \begin{itemize}\setlength\itemsep{0.4em}
    \item \alert{Rastrigin ($d{=}2$, escape problem):}
          SPGD \textbf{63\%} vs RPGD \textbf{20\%} --- same compute, same
          mechanism, only the selection rule differs. \alert{Cleanest
          evidence in this experiment that the steepest-selection rule
          does real work.} SGD/Adam stuck at 7\%; PGD at 0\% (single
          Gaussian sample isn't enough to find an escape direction).
    \item \alert{Ackley ($d{=}10$, mixed):} success rate is uninformative
          at $d{=}2$ (everyone $\sim$17\%). At $d{=}10$, \textbf{SPGD's
          mean final loss is $-2.57$ below RPGD's} on a paired comparison
          --- the binary success criterion doesn't fire here, but the
          loss-value gap is the headline number from this experiment.
    \item \alert{Rosenbrock ($d{=}2$, pure speed):} no local minima --- so
          this is purely a question of who descends a curved valley
          faster. PGD \textbf{93\%} and SPGD \textbf{90\%} both dominate;
          RPGD partial at 57\%; Adam 20\%; SGD 3\% (gradient
          poorly aligned with the curved valley axis).
  \end{itemize}
  \vspace{0.5em}
  \begin{block}{Caveat carried into all later experiments}
    Constant per-coordinate \amp{} becomes destructive at $d{=}50$:
    total displacement scales as $\sqrt{d}$, so an amplitude tuned at
    $d{=}2$ is $\sqrt{50/2}\!\approx\!5\times$ too large at $d{=}50$.
    \alert{Per-parameter amplitude scaling}
    ($\amp_p \propto \mathrm{std}(\theta_p^{(0)})$) is used in all
